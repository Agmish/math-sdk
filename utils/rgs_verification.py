...
def verify_mode_volatility(name: str, math_stats: object) -> dict:
    """Check current Stake math bet-level volatility limits."""
    mode_limits = {"prob5k": 1e-2, "prob10k": 0.5e-2, "etl40b": 0.9, "etl10k": 0.8, "cvar": 800, "rtp": 0.967}
    violated_attributes = {}
    for key, limit in mode_limits.items():
        value = getattr(math_stats, key, None)
        if value is not None and value > limit:
            violated_attributes[key] = value
    if violated_attributes:
        warnings.warn(f"\nMode [{name}] fails 3-star volatility limits:\n")
        for key, value in violated_attributes.items():
            print(f"\tVIOLATED: {key}  VALUE:{round(value, 4)} --- LIMIT: {mode_limits[key]}")
        print("\n\n")
